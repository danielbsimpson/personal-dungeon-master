...
# ---------------------------------------------------------------------------
# Helper: construct LlamaCppProvider with _check_reachable bypassed.
# ---------------------------------------------------------------------------

def _make_provider(settings: _MockSettings | None = None) -> LlamaCppProvider:
    """Create a LlamaCppProvider without actually connecting to the server."""
    if settings is None:
        settings = _MockSettings()
    with patch.object(LlamaCppProvider, "_check_reachable"), \
         patch("src.llm.llamacpp_provider.OpenAI"):
...
class TestFactory:
    def test_returns_llamacpp_provider(self) -> None:
        """create_provider returns a LlamaCppProvider when LLM_PROVIDER=llamacpp."""
        settings = _MockSettings(dm_model="llama3")

        with patch("src.llm.factory.LlamaCppProvider") as MockProvider:
    ...
    def test_raises_for_unsupported_provider(self) -> None:
        """create_provider raises NotImplementedError for any non-llamacpp value."""
...
class TestLlamaCppProviderListModels:
    def test_parses_v1_models_response(self) -> None:
        """list_models correctly parses the /v1/models JSON response."""
        provider = _make_provider()

        mock_response = MagicMock()
        mock_response.json.return_value = {
            "data": [
                {"id": "models/llama3.gguf", "meta": {"n_ctx_train": 8192}},
                {"id": "models/mistral.gguf", "meta": {"n_ctx_train": 32768}},
            ]
        }
        ...
        with patch("src.llm.llamacpp_provider.httpx.Client") as MockClient:
        ...
        assert len(models) == 2
        assert models[0].name == "models/llama3.gguf"
        assert models[0].context_window == 8192
        assert models[1].name == "models/mistral.gguf"
        assert models[1].context_window == 32768
        assert all(m.provider == "llamacpp" for m in models)

    def test_returns_empty_list_when_no_models(self) -> None:
        """list_models returns an empty list when no models are loaded."""
        provider = _make_provider()
...
class TestLlamaCppProviderReachability:
    def test_missing_server_raises_runtime_error(self) -> None:
        """LlamaCppProvider.__init__ raises RuntimeError when the server is not running."""
        settings = _MockSettings()

        mock_client = MagicMock()
        mock_client.get.side_effect = httpx.ConnectError("Connection refused")

        with patch("src.llm.llamacpp_provider.OpenAI"), \
             patch("src.llm.llamacpp_provider.httpx.Client") as MockClient:
            MockClient.return_value.__enter__ = lambda _self: mock_client
            MockClient.return_value.__exit__ = MagicMock(return_value=False)
    ...
    def test_timeout_raises_runtime_error(self) -> None:
        """LlamaCppProvider.__init__ raises RuntimeError when the server times out."""
        settings = _MockSettings()
